...
def init_db():
    ...
    try:
        ...
        logger.error(f"❌ Database initialization error: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def load_data():
    ...
    try:
        ...
        logger.error(f"❌ Error loading data: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def is_bot_running(script_owner_id, file_name): # Parameter renamed for clarity
    ...
    if script_info and script_info.get('process'):
        try:
            ...
            logger.error(f"Error checking process status for {script_key}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
            return False
    return False
...
def kill_process_tree(process_info):
    ...
    try:
                ...
                try:
                    ...
                    for child in children:
                        try:
                            ...
                            logger.error(f"Error terminating child {child.pid} for {script_key}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}. Trying kill...")
                    ...
                    for p in alive:
                        ...
                        try: p.kill()
                        except Exception as e: logger.error(f"Failed to kill child {p.pid} for {script_key} after wait: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
                    ...
                    try:
                        ...
                        logger.error(f"Error terminating parent {pid} for {script_key}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}. Trying kill...")
        ...
        logger.error(f"❌ Unexpected error killing process tree for PID {pid or 'N/A'} ({script_key}): {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def attempt_install_pip(module_name, message):
    ...
    try:
        ...
        error_msg = f"{E('cross')} Error installing <code>{package_name}</code>: {str(e).replace("<", "&lt;").replace(">", "&gt;")}"
...
def attempt_install_npm(module_name, user_folder, message):
    try:
        ...
        error_msg = f"{E('cross')} Error installing Node package <code>{module_name}</code>: {str(e).replace("<", "&lt;").replace(">", "&gt;")}"
        logger.error(error_msg, exc_info=True)
        bot.reply_to(message, error_msg)
        return False
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            ...
            try:
                ...
                if return_code != 0 and stderr:
                    ...
                    if match_py:
                        ...
                        if attempt_install_pip(module_name, message_obj_for_reply):
                         ...
                         error_summary = stderr[:500].replace('<', '&lt;').replace('>', '&gt;')
                ...
                logger.info("Python Check process killed. Proceeding to long run.")
                 ...
                 logger.error(f"Error in Python pre-check for {script_key}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
                 bot.reply_to(message_obj_for_reply, f"{E('cross')} Unexpected error in script pre-check for '{file_name}': {str(e).replace('<', '&lt;').replace('>', '&gt;')}")
                 return
            finally:
        ...
        try: log_file = open(log_file_path, 'w', encoding='utf-8', errors='ignore')
             ...
             logger.error(f"Failed to open log file '{log_file_path}' for {script_key}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
             bot.reply_to(message_obj_for_reply, f"{E('cross')} Failed to open log file '{log_file_path}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
             return
        try:
            ...
            if log_file and not log_file.closed: log_file.close()
            error_msg = f"{E('cross')} Error starting Python script '{file_name}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}"
            logger.error(error_msg, exc_info=True)
        ...
        error_msg = f"{E('cross')} Unexpected error running Python script '{file_name}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}"
        logger.error(error_msg, exc_info=True)
        bot.reply_to(message_obj_for_reply, error_msg)
        if script_key in bot_scripts:
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            ...
            try:
                ...
                logger.info("JS Check process killed. Proceeding to long run.")
                 ...
                 logger.error(f"Error in JS pre-check for {script_key}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
                 bot.reply_to(message_obj_for_reply, f"{E('cross')} Unexpected error in JS pre-check for '{file_name}': {str(e).replace('<', '&lt;').replace('>', '&gt;')}")
                 return
            finally:
                 if check_proc and check_proc.poll() is None:
        ...
        try: log_file = open(log_file_path, 'w', encoding='utf-8', errors='ignore')
            ...
            logger.error(f"Failed to open log file '{log_file_path}' for JS script {script_key}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
            bot.reply_to(message_obj_for_reply, f"{E('cross')} Failed to open log file '{log_file_path}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
            return
        try:
            startupinfo = None; creationflags = 0
            ...
            if log_file and not log_file.closed: log_file.close()
            error_msg = f"{E('cross')} Error starting JS script '{file_name}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}"
            logger.error(error_msg, exc_info=True)
            bot.reply_to(message_obj_for_reply, error_msg)
            if process and process.poll() is None:
        ...
        error_msg = f"{E('cross')} Unexpected error running JS script '{file_name}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}"
        logger.error(error_msg, exc_info=True)
        bot.reply_to(message_obj_for_reply, error_msg)
        if script_key in bot_scripts:
...
def check_force_join(user_id):
    ...
    try:
        ...
        logger.warning(f"Force-join check failed for user {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def save_user_file(user_id, file_name, file_type='py'):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite error saving file for user {user_id}, {file_name}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        except Exception as e: logger.error(f"❌ Unexpected error saving file for {user_id}, {file_name}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        finally: conn.close()
...
def remove_user_file_db(user_id, file_name):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite error removing file for {user_id}, {file_name}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        except Exception as e: logger.error(f"❌ Unexpected error removing file for {user_id}, {file_name}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        finally: conn.close()
...
def add_active_user(user_id):
    ...
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite error adding active user {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        except Exception as e: logger.error(f"❌ Unexpected error adding active user {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        finally: conn.close()
...
def save_subscription(user_id, expiry):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite error saving subscription for {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        except Exception as e: logger.error(f"❌ Unexpected error saving subscription for {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        finally: conn.close()
...
def remove_subscription_db(user_id):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite error removing subscription for {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        except Exception as e: logger.error(f"❌ Unexpected error removing subscription for {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        finally: conn.close()
...
def add_admin_db(admin_id):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite error adding admin {admin_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        except Exception as e: logger.error(f"❌ Unexpected error adding admin {admin_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        finally: conn.close()
...
def remove_admin_db(admin_id):
    ...
    with DB_LOCK:
        ...
        try:
            ...
            return removed
        except sqlite3.Error as e: logger.error(f"❌ SQLite error removing admin {admin_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}"); return False
        except Exception as e: logger.error(f"❌ Unexpected error removing admin {admin_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True); return False
        finally: conn.close()
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        if req_file:
            ...
            install_status_msg = bot.reply_to(message, f"{E('refresh')} Installing Python deps from <code>{req_file}</code>... (This may take a moment)", parse_mode='HTML')
            ...
            def do_pip_install():
                try:
                    command = [sys.executable, '-m', 'pip', 'install', '-r', req_path, '--no-cache-dir']
                    result = subprocess.run(command, capture_output=True, text=True, check=False, encoding='utf-8', errors='ignore', timeout=300)
                    if result.returncode == 0:
                        logger.info(f"pip install from requirements.txt OK. Output:\n{result.stdout}")
                        install_success[0] = True
                    else:
                        install_error[0] = result.stderr or result.stdout
                        logger.error(f"pip install failed. Output:\n{install_error[0]}")
                except subprocess.TimeoutExpired:
                    install_error[0] = "Installation timed out after 5 minutes."
                    logger.error("pip install timed out.")
                except Exception as e:
                    install_error[0] = str(e)
                    logger.error(f"pip install unexpected error: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
                finally:
                    install_done.set()

            pip_thread = threading.Thread(target=do_pip_install, daemon=True)
            pip_thread.start()
            install_done.wait(timeout=310)  # Wait max 310s for thread

            if install_success[0]:
                try:
                    bot.edit_message_text(f"{E('check')} Python deps from <code>{req_file}</code> installed successfully.", message.chat.id, install_status_msg.message_id, parse_mode='HTML')
                except Exception:
                    bot.reply_to(message, f"{E('check')} Python deps installed.", parse_mode='HTML')
            else:
                err = install_error[0] or "Unknown error"
                error_msg = f"{E('cross')} Failed to install Python deps from <code>{req_file}</code>.\nLog:\n<code>{err[:3500]}</code>"
                if len(error_msg) > 4000: error_msg = error_msg[:4000] + "\n... (Log truncated)"
                try:
                    bot.edit_message_text(error_msg, message.chat.id, install_status_msg.message_id, parse_mode='HTML')
                except Exception:
                    bot.reply_to(message, error_msg, parse_mode='HTML')
                return
        ...
        if pkg_json:
            ...
            npm_status_msg = bot.reply_to(message, f"{E('refresh')} Installing Node deps from <code>{pkg_json}</code>... (This may take a moment)", parse_mode='HTML')
            npm_success = [False]
            npm_error = [None]
            ...
            def do_npm_install():
                try:
                    command = ['npm', 'install']
                    result = subprocess.run(command, capture_output=True, text=True, check=False, cwd=temp_dir, encoding='utf-8', errors='ignore', timeout=300)
                    if result.returncode == 0:
                        logger.info(f"npm install OK. Output:\n{result.stdout}")
                        npm_success[0] = True
                    else:
                        npm_error[0] = result.stderr or result.stdout
                        logger.error(f"npm install failed. Output:\n{npm_error[0]}")
                except subprocess.TimeoutExpired:
                    npm_error[0] = "npm install timed out after 5 minutes."
                    logger.error("npm install timed out.")
                except FileNotFoundError:
                    npm_error[0] = "'npm' not found. Ensure Node.js/npm are installed and in PATH."
                    logger.error("npm not found.")
                except Exception as e:
                    npm_error[0] = str(e)
                    logger.error(f"npm install unexpected error: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
                finally:
                    npm_done.set()

            npm_thread = threading.Thread(target=do_npm_install, daemon=True)
            npm_thread.start()
            npm_done.wait(timeout=310)

            if npm_success[0]:
                try:
                    bot.edit_message_text(f"{E('check')} Node deps from <code>{pkg_json}</code> installed successfully.", message.chat.id, npm_status_msg.message_id, parse_mode='HTML')
                except Exception:
                    bot.reply_to(message, f"{E('check')} Node deps installed.", parse_mode='HTML')
            else:
                err = npm_error[0] or "Unknown error"
                error_msg = f"{E('cross')} Failed to install Node deps from <code>{pkg_json}</code>.\n<code>{err[:3500]}</code>"
                if len(error_msg) > 4000: error_msg = error_msg[:4000] + "\n... (Log truncated)"
                try:
                    bot.edit_message_text(error_msg, message.chat.id, npm_status_msg.message_id, parse_mode='HTML')
                except Exception:
                    bot.reply_to(message, error_msg, parse_mode='HTML')
                return
        ...
        logger.error(f"Bad zip file from {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        bot.reply_to(message, f"{E('cross')} Error: Invalid/corrupted ZIP. {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
    except Exception as e:
        logger.error(f"❌ Error processing zip for {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        bot.reply_to(message, f"{E('cross')} Error processing zip: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
    finally:
        if temp_dir and os.path.exists(temp_dir):
            try: shutil.rmtree(temp_dir); logger.info(f"Cleaned temp dir: {temp_dir}")
            except Exception as e: logger.error(f"Failed to clean temp dir {temp_dir}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def handle_js_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        ...
        logger.error(f"❌ Error processing JS file {file_name} for {script_owner_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        bot.reply_to(message, f"{E('cross')} Error processing JS file: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def handle_py_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        ...
        logger.error(f"❌ Error processing Python file {file_name} for {script_owner_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        bot.reply_to(message, f"{E('cross')} Error processing Python file: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
# --- End File Handling ---
...
def _logic_send_welcome(message):
    ...
    if user_id not in active_users:
        ...
        except Exception as e: logger.error(f"⚠️ Failed to notify owner about new user {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
    ...
    try:
        ...
        logger.error(f"Error sending welcome to {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        logger.error(f"Error during speed test (cmd): {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
            ...
            if not is_bot_running(target_user_id, file_name):
                ...
                if os.path.exists(file_path):
                    ...
                    try:
                        ...
                        logger.error(f"Error queueing start for '{file_name}' (user {target_user_id}): {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def handle_file_upload_doc(message): # Renamed
    ...
    try:
        ...
        except Exception as e: logger.error(f"Failed to forward uploaded file to OWNER_ID {OWNER_ID}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        ...
        if file_ext == '.zip':
         ...
         logger.error(f"Telegram API Error handling file for {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
         if "file is too big" in str(e).lower():
              bot.reply_to(message, f"{E('cross')} Telegram API Error: File too large to download (~20MB limit).")
         else: bot.reply_to(message, f"{E('cross')} Telegram API Error: {str(e).replace("<", "&lt;").replace(">", "&gt;")}. Try later.")
    except Exception as e:
        logger.error(f"❌ General error handling file for {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
        bot.reply_to(message, f"{E('cross')} Unexpected error: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def handle_callbacks(call):
    ...
    try:
        ...
        logger.error(f"Error handling callback '{data}' for {user_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def check_files_callback(call):
    ...
    if not user_files_list:
        ...
        except Exception as e: logger.error(f"Error editing msg for empty file list: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        return
    ...
    try:
        ...
        else: logger.error(f"Error editing msg for file list: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
    except Exception as e: logger.error(f"Unexpected error editing msg for file list: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
f'{PE("refresh")} Refresh Status', callback_data=f'file_{script_owner_id}_{file_name}'))
        ...
        logger.error(f"Error in file_control_callback for data '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def start_bot_callback(call):
    try:
        ...
        if is_bot_running(script_owner_id, file_name):
            ...
            except Exception as e: logger.error(f"Error updating buttons (already running): {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
            return
        ...
        logger.error(f"Error parsing start callback '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        bot.answer_callback_query(call.id, "Error: Invalid start command.", show_alert=True)
    except Exception as e:
        logger.error(f"Error in start_bot_callback for '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
    ...
    try:
        ...
        if not is_bot_running(script_owner_id, file_name): 
            ...
            try:
                ...
                bot.edit_message_text(card, chat_id_for_reply, call.message.message_id,
                                      reply_markup=ctrl_markup, parse_mode='HTML')
            except Exception as e: logger.error(f"Error updating buttons (already stopped): {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
            return
        ...
        logger.error(f"Error parsing stop callback '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        bot.answer_callback_query(call.id, "Error: Invalid stop command.", show_alert=True)
    except Exception as e:
        logger.error(f"Error in stop_bot_callback for '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def restart_bot_callback(call):
    try:
        ...
        logger.error(f"Error parsing restart callback '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        bot.answer_callback_query(call.id, "Error: Invalid restart command.", show_alert=True)
    except Exception as e:
        logger.error(f"Error in restart_bot_callback for '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def delete_bot_callback(call):
    try:
        ...
        if os.path.exists(file_path):
            ...
            except OSError as e: logger.error(f"Error deleting {file_path}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        if os.path.exists(log_path):
            try: os.remove(log_path); deleted_disk.append(os.path.basename(log_path)); logger.info(f"Deleted log: {log_path}")
            except OSError as e: logger.error(f"Error deleting log {log_path}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        ...
        try:
            ...
            logger.error(f"Error editing msg after delete: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
            bot.send_message(chat_id_for_reply, f"🗑️ <code>{file_name}</code> deleted.", reply_markup=back_markup, parse_mode='HTML')
    except (ValueError, IndexError) as e:
        logger.error(f"Error parsing delete callback '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        bot.answer_callback_query(call.id, "Error: Invalid delete command.", show_alert=True)
    except Exception as e:
        logger.error(f"Error in delete_bot_callback for '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def logs_bot_callback(call):
    try:
        ...
        try:
            ...
            logger.error(f"Error reading/sending log {log_path}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
            bot.send_message(chat_id_for_reply, f"{E('cross')} Error reading log for <code>{file_name}</code>.")
    except (ValueError, IndexError) as e:
        logger.error(f"Error parsing logs callback '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        bot.answer_callback_query(call.id, "Error: Invalid logs command.", show_alert=True)
    except Exception as e:
        logger.error(f"Error in logs_bot_callback for '{call.data}': {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def speed_callback(call):
    ...
    try:
        ...
        logger.error(f"Error during speed test (cb): {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def back_to_main_callback(call):
    ...
    try:
        ...
        bot.edit_message_text(main_menu_text, chat_id, call.message.message_id,
         ...
         else: logger.error(f"API error on back_to_main: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
    except Exception as e: logger.error(f"Error handling back_to_main: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def subscription_management_callback(call):
    ...
    except Exception as e: logger.error(f"Error showing sub menu: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def stats_callback(call):
    ...
    try:
        ...
        if "message is not modified" not in str(e):
            logger.error(f"Error updating stats message: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def lock_bot_callback(call):
    ...
    except Exception as e: logger.error(f"Error updating menu (lock): {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def unlock_bot_callback(call):
    ...
    try: bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=create_main_menu_inline(call.from_user.id))
    except Exception as e: logger.error(f"Error updating menu (unlock): {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def handle_confirm_broadcast(call):
    ...
    try:
        ...
        logger.error(f"Error in handle_confirm_broadcast: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
...
def execute_broadcast(broadcast_text, photo_id, video_id, caption, admin_chat_id):
    ...
    for i, user_id_bc in enumerate(users_to_broadcast): # Renamed
        try:
            ...
            else: logger.error(f"Broadcast failed to {user_id_bc}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}"); failed_count += 1
        except Exception as e: logger.error(f"Unexpected error broadcasting to {user_id_bc}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}"); failed_count += 1
    ...
    except Exception as e: logger.error(f"Failed to send broadcast result to admin {admin_chat_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def admin_panel_callback(call):
    ...
    except Exception as e: logger.error(f"Error showing admin panel: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def process_add_admin_id(message):
    ...
    try:
        ...
        except Exception as e: logger.error(f"Failed to notify new admin {new_admin_id}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
    except ValueError:
        ...
        bot.register_next_step_handler(msg, process_add_admin_id)
    except Exception as e: logger.error(f"Error processing add admin: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True); bot.reply_to(message, "Error.")
...
def process_remove_admin_id(message):
    ...
    try:
        ...
        if remove_admin_db(admin_id_remove): 
            ...
            except Exception as e: logger.error(f"Failed to notify removed admin {admin_id_remove}: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
        ...
        bot.register_next_step_handler(msg, process_remove_admin_id)
    except Exception as e: logger.error(f"Error processing remove admin: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True); bot.reply_to(message, "Error.")
...
def list_admins_callback(call):
    ...
    except Exception as e: logger.error(f"Error listing admins: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
...
def process_add_subscription_details(message):
    ...
    try:
        ...
        except Exception as e: logger.error(f"Failed to notify {sub_user_id} of new sub: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
    except ValueError as e:
        bot.reply_to(message, f"{E('warn')} Invalid: {str(e).replace("<", "&lt;").replace(">", "&gt;")}. Format: <code>ID days</code> or /cancel.")
        msg = bot.send_message(message.chat.id, f"{E('money')} Enter User ID & days, or /cancel.")
        bot.register_next_step_handler(msg, process_add_subscription_details)
    except Exception as e: logger.error(f"Error processing add sub: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True); bot.reply_to(message, "Error.")
...
def process_remove_subscription_id(message):
    ...
    try:
        ...
        except Exception as e: logger.error(f"Failed to notify {sub_user_id_remove} of sub removal: {str(e).replace("<", "&lt;").replace(">", "&gt;")}")
    except ValueError:
        bot.reply_to(message, f"{E('warn')} Invalid ID. Send numerical ID or /cancel.")
        msg = bot.send_message(message.chat.id, f"{E('money')} Enter User ID to remove sub from, or /cancel.")
        bot.register_next_step_handler(msg, process_remove_subscription_id)
    except Exception as e: logger.error(f"Error processing remove sub: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True); bot.reply_to(message, "Error.")
...
def process_check_subscription_id(message):
    ...
    try:
        ...
        bot.register_next_step_handler(msg, process_check_subscription_id)
    except Exception as e: logger.error(f"Error processing check sub: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True); bot.reply_to(message, "Error.")
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            logger.critical(f"{PE('bomb')} Unrecoverable polling error: {str(e).replace("<", "&lt;").replace(">", "&gt;")}", exc_info=True)
